...
const useCategorySubmit = (id, data) => {
  ...
  const onSubmit = async ({ name, description, index }) => {
    try {
      ...
      const categoryData = {
        ...
        index:
          index !== undefined
            ? Number(index)
            : Number(
                resData && resData.index !== undefined ? resData.index : 0,
              ),
        lang: language,
      };
      ...
      if (id) {
        ...
        // if `index` provided from form, override default
        if (
          typeof categoryData.index === "number" &&
      ...
      (async () => {
        try {
          ...
          if (res) {
            ...
            setValue("index", res.index !== undefined ? res.index : 0);
            setValue("language", language);
