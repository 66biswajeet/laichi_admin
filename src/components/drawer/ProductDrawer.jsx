...
const ProductDrawer = ({ id }) => {
  ...
  // Handler to update traditional variants (used by designer settings)
  const handleUpdateVariant = (variantIndex, updatedVariant) => {
    console.log("[ProductDrawer] handleUpdateVariant called:", {
      variantIndex,
      updatedVariant,
    ...
    if (sizeVariants.length > 0) {
      ...
      handleUpdateSizeVariant(variantIndex, updatedVariant);
    } else {
      // Update traditional variants
      const updatedVariants = [...variants];
      updatedVariants[variantIndex] = updatedVariant;
      console.log("[ProductDrawer] Updating traditional variants:", {
        before: variants,
        after: updatedVariants,
      });
                              ...
                              {attributes
                                ...
                                .map((attribute, i) => (
                                  <div key={attribute._id}>
                                    <div className="flex w-full h-10 justify-between font-sans rounded-tl rounded-tr bg-blue-100 dark:bg-blue-900 px-4 py-3 text-left text-sm font-normal text-gray-700 dark:text-gray-200">
                                      {"Select "}
                  ...
                  return (
                    <>
                      <div className="grid md:grid-cols-4 sm:grid-cols-2 grid-cols-1 gap-3 md:gap-3 xl:gap-3 lg:gap-2 mb-3">
                        <MultiSelect
                          options={attTitle?.filter((opt) => {
                            const label =
                              typeof opt?.label === "string"
                                ? opt.label
                                : opt?.label?.en || "";
                            return !label.toLowerCase().includes("size");
                          })}
                          value={attributes}
