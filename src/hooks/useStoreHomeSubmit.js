...
const useStoreHomeSubmit = () => {
  ...
  const [serviceMenuLink, setServiceMenuLink] = useState(false);
  const [helpMenuLink, setHelpMenuLink] = useState(false);
  const [serviceElements, setServiceElements] = useState([]);
  const [helpElements, setHelpElements] = useState([]);
  ...
  // CTA Hero states
  const [ctaHeroEnabled, setCtaHeroEnabled] = useState(true);
  const [ctaHeroBgImage, setCtaHeroBgImage] = useState("");

  // Featured Category Images state - stores custom images for each category
  const [featuredCategoryImages, setFeaturedCategoryImages] = useState({});
  ...
  const handleRemoveTestimonial = (index) => {
    const newList = testimonialsList.filter((_, i) => i !== index);
    setTestimonialsList(newList);
  };

  const handleToggleTestimonialVerified = (index) => {
    const newList = [...testimonialsList];
    newList[index].verified = !newList[index].verified;
    setTestimonialsList(newList);
  };

  // Service elements handlers
  const handleAddServiceElement = () => {
    setServiceElements([
      ...serviceElements,
      {
        name: {},
  ...
  const handleRemoveServiceElement = (index) => {
    const newList = serviceElements.filter((_, i) => i !== index);
    setServiceElements(newList);
  };

  const handleServiceElementImageChange = (index, url) => {
    const newList = [...serviceElements];
    newList[index].image = url;
    setServiceElements(newList);
  };

  // Help elements handlers
  const handleAddHelpElement = () => {
    setHelpElements([
      ...helpElements,
      {
        name: {},
  ...
  const handleRemoveHelpElement = (index) => {
    const newList = helpElements.filter((_, i) => i !== index);
    setHelpElements(newList);
  };

  const handleHelpElementImageChange = (index, url) => {
    const newList = [...helpElements];
    newList[index].image = url;
    setHelpElements(newList);
  };
  ...
  const handleRemoveFooterLink = (index) => {
    const newList = footerLinks.filter((_, i) => i !== index);
  ...
  // Featured Category Images handler
  const handleFeaturedCategoryImageChange = (categoryId, url) => {
    setFeaturedCategoryImages((prev) => ({
      ...prev,
      [categoryId]: {
        ...prev[categoryId],
        image: url,
      ...
      const storeCustomizationSettingData = {
        ...
        setting: {
          navbar: {
            ...
            service_menu_status: serviceMenuLink,
            help_menu_status: helpMenuLink,
            ...
            service_display_name: handleRemoveEmptyKey({
              ...resData?.navbar?.service_display_name,
              [language]: data.service_display_name || "",
            }),
            service_elements: serviceElements.map((element, index) => ({
              name: handleRemoveEmptyKey({
                ...element.name,
                [language]: data[`service_element_name_${index}`] || "",
              }),
              image: element.image,
              link: data[`service_element_link_${index}`] || "",
            })),
            help_display_name: handleRemoveEmptyKey({
              ...resData?.navbar?.help_display_name,
              [language]: data.help_display_name || "",
            }),
            help_elements: helpElements.map((element, index) => ({
              name: handleRemoveEmptyKey({
                ...element.name,
                [language]: data[`help_element_name_${index}`] || "",
              }),
              image: element.image,
              link: data[`help_element_link_${index}`] || "",
            })),
          ...
          home: {
            ...
            testimonials_status: testimonialsEnabled,
            testimonials_main_title: handleRemoveEmptyKey({
              ...resData?.home?.testimonials_main_title,
              [language]: data.testimonials_main_title || "",
            }),
            testimonials_main_subtitle: handleRemoveEmptyKey({
              ...resData?.home?.testimonials_main_subtitle,
              [language]: data.testimonials_main_subtitle || "",
            }),
            testimonials: testimonialsList.map((testimonial, index) => ({
              title: handleRemoveEmptyKey({
                ...testimonial.title,
                [language]: data[`testimonial_${index}_title`] || "",
              }),
              text: handleRemoveEmptyKey({
                ...testimonial.text,
                [language]: data[`testimonial_${index}_text`] || "",
              }),
              customer_name: handleRemoveEmptyKey({
                ...testimonial.customer_name,
                [language]: data[`testimonial_${index}_customer_name`] || "",
              }),
              rating: parseInt(data[`testimonial_${index}_rating`]) || 5,
              verified: testimonial.verified,
              verified_text: handleRemoveEmptyKey({
                ...testimonial.verified_text,
                [language]: data[`testimonial_${index}_verified_text`] || "",
              }),
            })),

            // CTA Hero settings
            cta_hero_status: ctaHeroEnabled,
            cta_hero_bg_image: ctaHeroBgImage,
            cta_hero_title: handleRemoveEmptyKey({
              ...resData?.home?.cta_hero_title,
              [language]: data.cta_hero_title || "",
            }),
            cta_hero_subtitle: handleRemoveEmptyKey({
              ...resData?.home?.cta_hero_subtitle,
              [language]: data.cta_hero_subtitle || "",
            }),
            cta_hero_button_text: handleRemoveEmptyKey({
              ...resData?.home?.cta_hero_button_text,
              [language]: data.cta_hero_button_text || "",
            }),
            cta_hero_button_link:
              data.cta_hero_button_link || "/search?category=",

            // Featured Category Images
            featured_category_images: featuredCategoryImages,
          },
          about_us: {
          ...
          hero: {
            title: handleRemoveEmptyKey({
              ...resData?.hero?.title,
              [language]: data.hero_title || "",
            }),
            subtitle: handleRemoveEmptyKey({
              ...resData?.hero?.subtitle,
              [language]: data.hero_subtitle || "",
            }),
            primary_label: handleRemoveEmptyKey({
              ...resData?.hero?.primary_label,
              [language]: data.hero_primary_label || "",
            }),
            primary_link: data.hero_primary_link || "",
            secondary_label: handleRemoveEmptyKey({
              ...resData?.hero?.secondary_label,
              [language]: data.hero_secondary_label || "",
            }),
            secondary_link: data.hero_secondary_link || "",
            feature_one: handleRemoveEmptyKey({
              ...resData?.hero?.feature_one,
              [language]: data.hero_feature_one || "",
            }),
            feature_two: handleRemoveEmptyKey({
              ...resData?.hero?.feature_two,
              [language]: data.hero_feature_two || "",
            }),
            feature_three: handleRemoveEmptyKey({
              ...resData?.hero?.feature_three,
              [language]: data.hero_feature_three || "",
            }),
            image: heroImage || "",
          },
        },
    ...
    const getStoreCustomizationData = async () => {
      try {
        ...
        if (res) {
          ...
          setServiceMenuLink(res?.navbar?.service_menu_status || false);
          setHelpMenuLink(res?.navbar?.help_menu_status || false);
          ...
          // Service and Help configuration
          setValue(
            "service_display_name",
            res?.navbar?.service_display_name?.[language || "en"] || "",
          );
          setValue(
            "help_display_name",
            res?.navbar?.help_display_name?.[language || "en"] || "",
          );

          // Load service elements
          if (res?.navbar?.service_elements?.length > 0) {
            setServiceElements(res.navbar.service_elements);
            res.navbar.service_elements.forEach((element, index) => {
              setValue(
                `service_element_name_${index}`,
                element.name?.[language || "en"] || "",
              );
              setValue(`service_element_link_${index}`, element.link || "");
            });
          }

          // Load help elements
          if (res?.navbar?.help_elements?.length > 0) {
            setHelpElements(res.navbar.help_elements);
            res.navbar.help_elements.forEach((element, index) => {
              setValue(
                `help_element_name_${index}`,
                element.name?.[language || "en"] || "",
              );
              setValue(`help_element_link_${index}`, element.link || "");
            });
          }
          ...
          // hero fields
          setValue("hero_title", res?.hero?.title?.[language || "en"] || "");
          setValue(
            "hero_subtitle",
            res?.hero?.subtitle?.[language || "en"] || "",
          );
          setValue(
            "hero_primary_label",
            res?.hero?.primary_label?.[language || "en"] || "",
          );
          setValue("hero_primary_link", res?.hero?.primary_link || "");
          setValue(
            "hero_secondary_label",
            res?.hero?.secondary_label?.[language || "en"] || "",
          );
          setValue("hero_secondary_link", res?.hero?.secondary_link || "");
          setValue(
            "hero_feature_one",
            res?.hero?.feature_one?.[language || "en"] || "",
          );
          setValue(
            "hero_feature_two",
            res?.hero?.feature_two?.[language || "en"] || "",
          );
          setValue(
            "hero_feature_three",
            res?.hero?.feature_three?.[language || "en"] || "",
          );
          ...
          // testimonials
          setTestimonialsEnabled(res?.home?.testimonials_status !== false);
          setValue(
            "testimonials_main_title",
            res?.home?.testimonials_main_title?.[language || "en"] || "",
          );
          setValue(
            "testimonials_main_subtitle",
            res?.home?.testimonials_main_subtitle?.[language || "en"] || "",
          );

          // Load testimonials list
          const loadedTestimonials = res?.home?.testimonials || [];
          setTestimonialsList(loadedTestimonials);

          // Set form values for testimonials
          loadedTestimonials.forEach((testimonial, index) => {
            setValue(
              `testimonial_${index}_title`,
              testimonial.title?.[language || "en"] || "",
            );
            setValue(
              `testimonial_${index}_text`,
              testimonial.text?.[language || "en"] || "",
            );
            setValue(
              `testimonial_${index}_customer_name`,
              testimonial.customer_name?.[language || "en"] || "",
            );
            setValue(`testimonial_${index}_rating`, testimonial.rating || 5);
            setValue(
              `testimonial_${index}_verified_text`,
              testimonial.verified_text?.[language || "en"] || "",
            );
          });

          // CTA Hero
          setCtaHeroEnabled(res?.home?.cta_hero_status !== false);
          setCtaHeroBgImage(res?.home?.cta_hero_bg_image || "");
          setValue(
            "cta_hero_title",
            res?.home?.cta_hero_title?.[language || "en"] || "",
          );
          setValue(
            "cta_hero_subtitle",
            res?.home?.cta_hero_subtitle?.[language || "en"] || "",
          );
          setValue(
            "cta_hero_button_text",
            res?.home?.cta_hero_button_text?.[language || "en"] || "",
          );
          setValue(
            "cta_hero_button_link",
            res?.home?.cta_hero_button_link || "/search?category=",
          );

          // Load featured category images
          const savedCategoryImages = res?.home?.featured_category_images || {};

          // Fetch categories to populate the selector
          try {
            const categories = await CategoryServices.getAllCategory();
            const topLevelCategories = categories?.[0]?.children || [];

            // Initialize category images mapping
            const categoryImagesMap = {};
            topLevelCategories.forEach((category) => {
              categoryImagesMap[category._id] = {
                name: category.name?.en || category.name,
                image:
                  savedCategoryImages[category._id]?.image ||
                  category.icon ||
                  "",
              };
            });

            setFeaturedCategoryImages(categoryImagesMap);
          } catch (error) {
            console.error("Failed to load categories:", error);
            // If categories fail to load, just set the saved images
            setFeaturedCategoryImages(savedCategoryImages);
          }
          ...
          // Load footer links
          if (
            res?.footer?.footer_links &&
            Array.isArray(res.footer.footer_links)
          ) {
            setFooterLinks(res.footer.footer_links);
          }
